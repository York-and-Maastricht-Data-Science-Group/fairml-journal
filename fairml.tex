...
%In \textit{preprocessing}, users measure the initial biases of an original dataset (\textit{Measure initial bias} action). The measurement can produce several results especially when more than one metric are used. The results are presented in numbers and charts to help usedrs understand the initial biases. After that, the original dataset is copied, and the copy is transformed using a bias mitigation algorithm to produce a less-biased, transformed dataset (\textit{Mitigate bias} action). Bias mitigation in \textit{preprocessing} can be done without training any model and performing any prediction since the transformation to reduce biases is only applied to datasets. However, users can train a model and make prediction if they need to check the accuracies of the models trained from the original and mitigated datasets. 
%
%In \textit{inprocessing}, users train a model and make prediction using it. The original model and prediction  are used by bias metrics as parameters to measure original biases (in \textit{Measure initial bias} action). After that, in \textit{Mitigate bias} action, a bias mitigation algorithm is applied to the original model to reduce its bias producing a mitigated model. Afterwards, the model is used to make another prediction.
