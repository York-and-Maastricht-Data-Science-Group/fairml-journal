...
\copyrightyear{2022}
\acmYear{2022}
\setcopyright{acmcopyright}\acmConference[MODELS '22]{ACM/IEEE 25th
...
\acmBooktitle{ACM/IEEE 25th International Conference on Model Driven Engineering
	Languages and Systems (MODELS '22), October 23--28, 2022, Montreal, QC, Canada}
\acmPrice{15.00}
\acmDOI{10.1145/3550355.3552401}
\acmISBN{978-1-4503-9466-6/22/10}
...
\begin{document}
	...
	\acmBadgeL[https://www.acm.org/publications/policies/artifact-review-and-badging-current\#reusable]{images/artifacts_evaluated_reusable_v1_1}
	\acmBadgeR[https://www.acm.org/publications/policies/artifact-review-and-badging-current\#available]{images/artifacts_available_v1_1}
...
\end{lstlisting}
	...
	%	There are many available debiasing algorithms and bias metrics that users can select to mitigate biases in machine learning. Some of them are better than others in a certain context. 
	%	FairML has a wizard feature that assists users in selecting the most appropriate debiasing algorithms and bias metrics based on their answers to the wizard's questions. 
...
\begin{lstlisting}[firstnumber=1,style=log,caption={Some of the questions asked in the FairML's wizard to assist users to select the best debiasing algorithms and bias metrics.},label=lst:wizard_questions]
...
...
\end{lstlisting}
	...
	\begin{table*}[]
		\caption{IBM AI Fairness 360 examples and the numbers of unique datasets, classifiers, debiasing algorithms, bias metrics, and measurement values in each example.}
	...
	%\begin{table*}[]
	%	\caption{Part 2 of 2 -- IBM AI Fairness 360 examples and the numbers of unique datasets, classifiers, debiasing algorithms, bias metrics, and measurement values in each example.}
...
%	This section is quite tricky as it can provide reviewers all the ammunition they need if they want to reject the paper :) If we switch to ``conciseness" instead of “productivity” there’s one fewer threat to consider. Also, we should repeat that even though we have not conducted experiments with users, the examples we reproduced were written by experts and the data they operate on are real-world data from a variety of domains.
...
%	First, user feedback is useful to gain insights into productivity as writing fewer lines of code to express bias mitigation model, productivity evaluation in Section \ref{sec:productivity}, does not necessarily mean better productivity due to different factors \cite{sadowski2019rethinking}. 
%	Second, user feedback is crucial to capture the workflows of bias mitigation in practice. IBM AI Fairness 360's examples can be too simple for real-world cases and therefore invalidate the expressiveness evaluation in Section \ref{sec:expressiveness_and_correctness}. These threats has been identified by \cite{lee2021landscape} as the adoption of fairness toolkits is still not a common practice.
	
%	How about (some) of the following lessons learnt?
%	
%	- Bias identification and mitigation is supported by well-understood algorithms that target different types of features and bias that can act as a solid foundation for a well-bounded DSL such as FairML
%	- A model-based solution such as FairML enables a clean separation of the core bias detection and mitigation process from peripheral (e.g. visualisation) code 
%	- Bias identification and mitigation processes are easy to get wrong (e.g. by combining incompatible algorithms). Expressing them in a DSL enables semantic error checking that is not possible at the Python level. 
%	- Generating Jupyter notebooks from models directly through model-to-text transformation or through a model-to-JSON transformation is technically demanding but can be avoided by leveraging 3rd party engines such as P2J, which can transform textual Python programs to Jupyter notebooks. (Not sure I’d mention protected regions as some reviewers won’t be familiar with them)
...
\begin{itemize}[leftmargin=*]
	...
	%	\item  Bias identification and mitigation processes are easy to get wrong (e.g., applying algorithms in the wrong orders). Expressing them in a DSL enables semantic error checking that is impossible at the Python level. 
...
\end{itemize}
%	As the adoption of fairness toolkits is still not a common practice \cite{lee2021landscape}, it was challenging for us to perform domain analysis and user evaluation to obtain feedback from experienced users in the field. As a solution, we opted for document analysis as documents contain information to understand the objects of interest \cite{bowen2009document}. We analysed the code repository and documentation of IBM AI Fairness 360, as they can reflect the bias mitigation in practice to understand its uses and implementation in mitigating bias in machine learning. Through the documentation and code repository of the toolkit, we identified the important constructs and workflows of bias mitigation in machine learning and used them to construct FairML. We used the examples provided by the toolkit as the benchmark for the correctness and performance evaluation of the generated bias mitigation code. 
%	
%	A technical challenge we encountered was in generating Jupyter notebook (IPYNB) files. We want to support users so that they can add their custom code into the generated notebooks or modify existing code without being overridden when the code generation is re-executed. Initially, we aimed to add protected regions into the IPYNB files. However, since an IPYNB file is a JSON file, it does not support \textit{comments}. The inability invalidated our approach that used EGL engine \cite{rose2008egl} for adding protected regions as the engine relies on commenting characters to mark protected areas. As a solution, we use Python files as the intermediate generated files, as Python code supports commenting, and a third-party P2J engine to covert the Python files to Jupyter notebook files. However, this solution is imperfect as users can only modify the Python files if they want their own code or modify existing code to be preserved.
