...
\begin{figure*}
	\includegraphics[width=\linewidth]{figures/table-output.png}
	\caption{The table ouput displayed in the generated Juypter notebook file based on the model defined in Listing \ref{lst:fairml_model}.}
...
Based on the model defined in Listing \ref{lst:fairml_model}, FairML generates a Jupyter notebook file containing the code that performs the bias mitigation, explanation of the bias measured, and references to the selected classifiers and debiasing algorithms to help users understand the applied bias mitigation and metrics measured. 

It also contains a table (Figure \ref{fig:table-output}) and chart that compare the bias metrics measured by different combination of datasets, classifiers, and debiasing algorithms to help users select the best combinations that fits with their contexts. 
...
\begin{table*}[]
	\caption{Comparison of numbers of lines of code between original examples (Original LoC), FairML models (Model LoC), and generated code (Generated LoC) as well as the generation time of FairML and the execution time of the original examples and generated code (seconds). See Tables \ref{tab:expressiveness1} and \ref{tab:expressiveness2} for the descriptions of the codes in the first column.}
