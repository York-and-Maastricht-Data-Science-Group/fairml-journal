...
the toolkit comes with a number of examples (Tables \ref{tab:loc} and \ref{tab:time}) that demonstrate 
...
We compared the bias metric values measured in the original examples with the values measured in the generated code to evaluate the correctness of FairML's generated code. Both should produce equal values or similar values, up to 10\% tolerance, when randomness is required.  
...
\subsection{Expressiveness and Accuracy}
\label{sec:expressiveness_and_accuracy}
Based on our evaluation, FairML was able to reproduce all the scenarios in all the 21 examples in Tables \ref{tab:loc} and \ref{tab:time}. In total, all the scenarios covered 14 different bias mitigation algorithms, 7 bias metrics, 7 datasets, 6 classifiers, and X measurement values.
